...
import TopDoctorSkeleton from '@/components/TopDoctorSkeleton'
...
const TopDoctor = () => {
  const {doctors,isLoading}=useFunc()
    const router=useRouter()

    console.log(doctors,'yoo')
  return (
      <div className='flex flex-col items-center gap-4 my-16 text-gray-800 md:mx-10'>
      <h2 className='text-3xl font-medium'>Top Doctor</h2>
      <p className='sm:w-1/3 text-center text-sm'>Simply browse through our extensive list of trusted doctors</p>
     {isLoading ? <TopDoctorSkeleton/> : <div className='grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-y-6 px-3 sm:px-0 gap-5 pt-5 w-full'>
        {doctors && doctors.slice(0,10).map((doctor,index)=>(
