        ...
        <div className={`flex-col gap-4 text-sm text-gray-600 ${showFilter? 'flex':'hidden sm:flex'}`}>
          {
            specialties.map((sp,index)=>(
               <p key={index} onClick={()=>{router.push(`/doctors/${sp}`)}} className={`w-[94vh] sm:w-auto pl-3 py-1.5 pr-16 border duration-300 border-gray-300 rounded transition-all cursor-pointer`}>{sp}</p>
            ))
          }
        </div>
